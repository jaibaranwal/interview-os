import { FeedbackObject, CandidateProfile } from '../types';
import { ConversationMemory } from '../memory/ConversationMemory';
import { CurriculumLoader } from '../data/CurriculumLoader';
import { ILLMClient, LLMClient } from './LLMClient';
...
export class FeedbackEngine implements IFeedbackEngine {
  ...
  public async generateFeedback(
    ...
    if (recordedStrengths.length > 0) {
      ...
      strengths.push(
        `Demonstrated solid engagement across ${visitedDays.length} evaluated cohort days (${visitedDayTitles.slice(0, 2).join(', ')}).`,
        `Effective communication of practical development experience in ${member.jobRole}.`
      );
    }
  ...
  "next": ["actionable next step 1", "actionable next step 2"]
      ...
      if (jsonMatch) {
        ...
        if (
          ...
          Logger.info(`Generated LLM-enhanced feedback for session.`);
          return parsed;
        }
      }
    } catch (err: any) {
      Logger.info(`LLM feedback enhancement skipped, using deterministic feedback engine output.`);
    }
