import { LLMEvaluationResult, CurriculumDay, CorrectnessClassification } from '../types';
...
export class ResponseEvaluator {
  ...
  public async evaluateResponse(
    ...
    if (fastCheck) {
      Logger.info(`Fast-path Evaluation: score=${fastCheck.score} classification=${fastCheck.correctness} action=${fastCheck.next_action}`);
      return fastCheck;
    }
...
Output ONLY valid JSON.`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        if (typeof parsed.score === 'number' && parsed.next_action && parsed.correctness) {
          ...
          const next_action = (score: number, origAction: string) => {
            if (score >= 60) return 'advance';
            if (score >= 20 || correctness === 'LACK_OF_EXPERIENCE') return 'follow_up';
            return 'retry';
          };

          Logger.info(`Evaluation: score=${parsed.score} classification=${correctness} action=${next_action(parsed.score, parsed.next_action)}`);
          return {
      ...
      Logger.error('LLM evaluation failed — using fallback classifier:', err.message);
      return this.fallbackEvaluate(trimmed);
    }
  }

  private performFastClassification(trimmed: string, previousAnswer?: string): LLMEvaluationResult | null {
    ...
    if (
      /\b(never used|haven't worked|havent worked|don't have experience|dont have experience|didn't implement|didnt implement|never done|no experience|never worked|haven't used|havent used)\b/i.test(lower) ||
    ...
    if (
      lower.includes("don't know") || lower.includes("dont know") ||
  ...
  private normalizeClassification(raw: string, score: number): CorrectnessClassification {
    const upper = (raw || '').toUpperCase();

    if (['PROFANITY', 'GIBBERISH', 'REFUSAL', 'OFF_TOPIC', 'UNCERTAIN', 'WEAK', 'GOOD', 'EXCELLENT', 'LACK_OF_EXPERIENCE'].includes(upper)) {
      return upper as CorrectnessClassification;
    }

    if (upper === 'EXEMPLARY') return 'EXCELLENT';
    if (upper === 'ADEQUATE') return 'GOOD';
    if (upper === 'INVALID' || upper === 'NO_ATTEMPT') return 'GIBBERISH';
  ...
  private fallbackEvaluate(trimmed: string): LLMEvaluationResult {
